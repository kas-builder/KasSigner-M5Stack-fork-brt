...
impl<'a> BootDisplay<'a> {
    ...
    pub fn set_security_test_state(
        ...
        if index >= LABELS.len() || state > 4 { return Err("Invalid security test state"); }
        ...
        for (i, label) in LABELS.iter().enumerate() {
            ...
            let (status, color) = match self.security_status[i] {
                1 => ("RUN", COLOR_ORANGE), 2 => ("PASS", KASPA_TEAL),
                3 => ("FAIL", COLOR_DANGER), 4 => ("DEV", COLOR_ORANGE),
                _ => ("--", COLOR_TEXT_DIM),
            };
